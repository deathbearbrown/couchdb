...
// can I get from active_tasks using type="replication"
  ...
  View.ReplicationForm = FauxtonAPI.View.extend({
		...
		startReplication: function(json){
			...
			this.newRepModel.save(json,{
				...
				error: function(resp){
					console.log("replication error", resp);
		...
		showProgress: function(formData){
			...
			progressBar = this.insertView("#replicationStatus", new View.replicationProgress({ 
				formData: formData,
				model: fetchStatus
      }));
      progressBar.render();
	...
	View.replicationProgress = FauxtonAPI.View.extend({
